...
namespace NGCP.UGV
{
    public partial class UGV
    {
      ...
      //  Comnet.Node protonet { get; set; }
        ...
        //commAuth rxCommState;

        //public byte AuthNodeID
        //{
        //    get
        //    {
        //        return (byte)rxCommState.AuthorizedNode;
        //    }
        //}
        ...
        public void Start()
        {
            ...
            */
            ...
            {
                TargetAngle = BitConverter.ToDouble(bytes, 0);
                TargetDistance = BitConverter.ToDouble(bytes, sizeof(double));
                if (TargetAngle >= 0 && TargetDistance >= 0)
                {
                    ...
                    TargetDistance *= 0.25;
                    TargetFound = true;
                    TargetWaypoint = WayPoint.Projection(new WayPoint(Latitude, Longitude, 0), Heading - TargetAngle, TargetDistance);
                }
                else
                    TargetFound = false;
            });
        ...
        void SendControl()
        {
            ...
            else if (Settings.DriveMode == DriveMode.SemiAutonomous)
            {
                ...
                FinalSteering = (Steering * 100 / 1000.0) + 512;
            }
            ...
            else if (Settings.DriveMode == DriveMode.LocalControl)
            {
                ...
                FinalFrontWheel = (LocalSpeed * 255.0 / 1000.0);
                FinalRearWheel = (LocalSpeed * 255.0 / 1000.0);
                FinalSteering = (LocalSteering * 100 / 1000.0) + 512;
            }
            ...
            if (Enabled)
            {
                ...
                FinalSteering = Math.Min(Math.Max(FinalSteering, 412), 612);
            }
            else
            {
                FinalFrontWheel = 0;
            ...
            byte[] ReverseSteeringAngle = BitConverter.GetBytes((Int16)(512.0 - (FinalSteering - 512.0)));
        ...
        public class UGVSetting
        {
            ...
            public int CommBaud = 115200;
            public int CommNode = 1;
            public bool UseXBeeComm = true;
            public bool UseProtonetComm = true;
